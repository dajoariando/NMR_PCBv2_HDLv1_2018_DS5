...
uint32_t i2c_tfr_cmd_fifo_lvl(volatile unsigned long * i2c_addr) {
	return (alt_read_word(i2c_addr + TFR_CMD_FIFO_LVL_OFST)
			& TFR_CMD_FIFO_LVL_MSK);
}

uint32_t i2c_rxdata_fifo_lvl(volatile unsigned long * i2c_addr) {
	return (alt_read_word(i2c_addr + RX_DATA_FIFO_LVL_OFST)
			& RX_DATA_FIFO_LVL_MSK);
}
...
void i2c_wait_transfer(volatile unsigned long * i2c_addr) {
	while (!((alt_read_word(i2c_addr + ISR_OFST) & TX_READY_MSK) == TX_READY_MSK))
		; // wait until TX_READY signal is asserted
}
...
uint32_t rd_hall_sens(uint8_t reg_addr, uint8_t en_mesg) { // read command for the hall sensor
	...
	alt_write_word((h2p_i2c_ext_addr + TFR_CMD_OFST),
			(1 << STA_SHFT) | (0 << STO_SHFT) | (i2c_addr_relay << AD_SHFT)
					| (WR_I2C << RW_D_SHFT)); //check_i2c_isr_stat (h2p_i2c_ext_addr, en_mesg);
	alt_write_word((h2p_i2c_ext_addr + TFR_CMD_OFST),
			(0 << STA_SHFT) | (0 << STO_SHFT) | (reg_addr & I2C_DATA_MSK)); //check_i2c_isr_stat (h2p_i2c_ext_addr, en_mesg);

	alt_write_word((h2p_i2c_ext_addr + TFR_CMD_OFST),
			(1 << STA_SHFT) | (0 << STO_SHFT) | (i2c_addr_relay << AD_SHFT)
					| (RD_I2C << RW_D_SHFT)); //check_i2c_isr_stat (h2p_i2c_ext_addr, en_mesg);
	alt_write_word((h2p_i2c_ext_addr + TFR_CMD_OFST),
			(0 << STA_SHFT) | (0 << STO_SHFT) | ((0x00) & I2C_DATA_MSK)); //check_i2c_isr_stat (h2p_i2c_ext_addr, en_mesg);
	alt_write_word((h2p_i2c_ext_addr + TFR_CMD_OFST),
			(0 << STA_SHFT) | (0 << STO_SHFT) | ((0x00) & I2C_DATA_MSK)); //check_i2c_isr_stat (h2p_i2c_ext_addr, en_mesg);
	alt_write_word((h2p_i2c_ext_addr + TFR_CMD_OFST),
			(0 << STA_SHFT) | (0 << STO_SHFT) | ((0x00) & I2C_DATA_MSK)); //check_i2c_isr_stat (h2p_i2c_ext_addr, en_mesg);
	alt_write_word((h2p_i2c_ext_addr + TFR_CMD_OFST),
			(0 << STA_SHFT) | (1 << STO_SHFT) | ((0x00) & I2C_DATA_MSK)); //check_i2c_isr_stat (h2p_i2c_ext_addr, en_mesg);
	...
	dataout = (dataout << 8)
			| (alt_read_word(h2p_i2c_ext_addr + RX_DATA_OFST) & RX_DATA_MSK); // read bit 31:24
	dataout = (dataout << 8)
			| (alt_read_word(h2p_i2c_ext_addr + RX_DATA_OFST) & RX_DATA_MSK); // read bit 23:16
	dataout = (dataout << 8)
			| (alt_read_word(h2p_i2c_ext_addr + RX_DATA_OFST) & RX_DATA_MSK); // read bit 15:8
	dataout = (dataout << 8)
			| (alt_read_word(h2p_i2c_ext_addr + RX_DATA_OFST) & RX_DATA_MSK); // read bit 7:0
...
void rd_hall_sens_stat(void) {
	...
	printf("\tEEP02_I2C_CRC_Enable = %d\n",
			(REG02 >> EEP02_I2C_CRC_Enable_OFST)
					& EEP02_I2C_CRC_Enable_BASEMASK);
	printf("\tEEP02_Disable_Slave_ADC = %d\n",
			(REG02 >> EEP02_Disable_Slave_ADC_OFST)
					& EEP02_Disable_Slave_ADC_BASEMASK);
	...
	printf("\tEEP02_ChannelZ_Enable = %d\n",
			(REG02 >> EEP02_ChannelZ_Enable_OFST)
					& EEP02_ChannelZ_Enable_BASEMASK);
	printf("\tEEP02_ChannelY_Enable = %d\n",
			(REG02 >> EEP02_ChannelY_Enable_OFST)
					& EEP02_ChannelY_Enable_BASEMASK);
	printf("\tEEP02_ChannelX_Enable = %d\n",
			(REG02 >> EEP02_ChannelX_Enable_OFST)
					& EEP02_ChannelX_Enable_BASEMASK);
	printf("\tEEP02_INT_Latch_Enable = %d\n",
			(REG02 >> EEP02_INT_Latch_Enable_OFST)
					& EEP02_INT_Latch_Enable_BASEMASK);
	printf("\tEEP02_Customer_EEPROM = %d\n",
			(REG02 >> EEP02_Customer_EEPROM_OFST)
					& EEP02_Customer_EEPROM_BASEMASK);

	printf("REG03 = 0x%x\n", REG03);
	printf("\tEEP03_Signed_INT_Enable   = %d\n",
			(REG03 >> EEP03_Signed_INT_Enable_OFST)
					& EEP03_Signed_INT_Enable_BASEMASK);
	printf("\tEEP03_INT_Mode            = %d\n",
			(REG03 >> EEP03_INT_Mode_OFST) & EEP03_INT_Mode_BASEMASK);
	printf("\tEEP03_INT_EEPROM_Status   = %d\n",
			(REG03 >> EEP03_INT_EEPROM_Status_OFST)
					& EEP03_INT_EEPROM_Status_BASEMASK);
	printf("\tEEP03_INT_EEPROM_Enable   = %d\n",
			(REG03 >> EEP03_INT_EEPROM_Enable_OFST)
					& EEP03_INT_EEPROM_Enable_BASEMASK);
	...
	printf("\tEEP03_Z_INT_Threshold     = %d\n",
			(REG03 >> EEP03_Z_INT_Threshold_OFST)
					& EEP03_Z_INT_Threshold_BASEMASK);
	printf("\tEEP03_Y_INT_Threshold     = %d\n",
			(REG03 >> EEP03_Y_INT_Threshold_OFST)
					& EEP03_Y_INT_Threshold_BASEMASK);
	printf("\tEEP03_X_INT_Threshold     = %d\n",
			(REG03 >> EEP03_X_INT_Threshold_OFST)
					& EEP03_X_INT_Threshold_BASEMASK);

	printf("REG27 = 0x%x\n", REG27);
	printf("\tVola27_Low_Power_Mode_Count_Max  = %d\n",
			(REG27 >> Vola27_Low_Power_Mode_Count_Max_OFST)
					& Vola27_Low_Power_Mode_Count_Max_BASEMASK);
	printf("\tVola27_I2C_Loop_Mode 			   = %d\n",
			(REG27 >> Vola27_I2C_Loop_Mode_OFST)
					& Vola27_I2C_Loop_Mode_BASEMASK);
	...
	printf("\tVola28_Temperature_MSBs = %d\n",
			(REG28 >> Vola28_Temperature_MSBs_OFST)
					& Vola28_Temperature_MSBs_BASEMASK);

	printf("REG29 = 0x%x\n", REG29);
	printf("\tVola29_Interrupt_Write    = %d\n",
			(REG29 >> Vola29_Interrupt_Write_OFST)
					& Vola29_Interrupt_Write_BASEMASK);
	...
	printf("\tVola29_Hall_Mode_Status   = %d\n",
			(REG29 >> Vola29_Hall_Mode_Status_OFST)
					& Vola29_Hall_Mode_Status_BASEMASK);
	printf("\tVola29_Temperature_LSBs   = %d\n",
			(REG29 >> Vola29_Temperature_LSBs_OFST)
					& Vola29_Temperature_LSBs_BASEMASK);
...
void rd_hall_sens_data(void) {
	...
	XDigit =
			(((REG28 >> Vola28_X_Axis_MSBs_OFST) & Vola28_X_Axis_MSBs_BASEMASK)
					<< 4)
					| ((REG29 >> Vola29_X_Axis_LSBs_OFST)
							& Vola29_X_Axis_LSBs_BASEMASK); // combine MSB and LSB
	YDigit =
			(((REG28 >> Vola28_Y_Axis_MSBs_OFST) & Vola28_Y_Axis_MSBs_BASEMASK)
					<< 4)
					| ((REG29 >> Vola29_Y_Axis_LSBs_OFST)
							& Vola29_Y_Axis_LSBs_BASEMASK); // combine MSB and LSB
	ZDigit =
			(((REG28 >> Vola28_Z_Axis_MSBs_OFST) & Vola28_Z_Axis_MSBs_BASEMASK)
					<< 4)
					| ((REG29 >> Vola29_Z_Axis_LSBs_OFST)
							& Vola29_Z_Axis_LSBs_BASEMASK); // combine MSB and LSB
	TempDigit = (((REG28 >> Vola28_Temperature_MSBs_OFST)
			& Vola28_Temperature_MSBs_BASEMASK) << 6)
			| ((REG29 >> Vola29_Temperature_LSBs_OFST)
					& Vola29_Temperature_LSBs_BASEMASK); // combine MSB and LSB
...
void write_i2c_pwm(uint8_t pwm_address, uint8_t pwm_data) {
	...
	alt_write_word((h2p_i2c_ext_addr + TFR_CMD_OFST),
			(1 << STA_SHFT) | (0 << STO_SHFT) | (i2c_addr_cnt << AD_SHFT)
					| (WR_I2C << RW_D_SHFT)); // send chip address
	alt_write_word((h2p_i2c_ext_addr + TFR_CMD_OFST),
			(0 << STA_SHFT) | (0 << STO_SHFT) | (pwm_address & I2C_DATA_MSK)); // send register address
	alt_write_word((h2p_i2c_ext_addr + TFR_CMD_OFST),
			(0 << STA_SHFT) | (1 << STO_SHFT) | ((pwm_data) & I2C_DATA_MSK)); // send data
}
...
void write_i2c_pwm_rst() {
	...
	alt_write_word((h2p_i2c_ext_addr + TFR_CMD_OFST),
			(1 << STA_SHFT) | (0 << STO_SHFT) | (i2c_addr_cnt << AD_SHFT)
					| (WR_I2C << RW_D_SHFT)); // send general address
	alt_write_word((h2p_i2c_ext_addr + TFR_CMD_OFST),
			(0 << STA_SHFT) | (1 << STO_SHFT) | (0x06 & I2C_DATA_MSK)); // send SWRST Command
	usleep(10000);
...
void fifo_to_sdram_dma_trf(uint32_t transfer_length) {
	...
	alt_write_word(h2p_dma_addr + DMA_CONTROL_OFST,
...
void datawrite_with_dma(uint32_t transfer_length, uint8_t en_mesg) {
	...
	do {
		...
		if (en_mesg) {
			...
			usleep(1); // wait time to prevent overloading the DMA bus arbitration request
		}
...
void CPMG_Sequence(double cpmg_freq, double pulse1_us, double pulse2_us,
	...
	if (!data_nowrite) { // write data to text with C programming
		if (read_with_dma) { // if read with dma is intended
			datawrite_with_dma(samples_per_echo * echoes_per_scan / 2,
					DISABLE_MESSAGE);
		...
		if (wr_indv_data) {
			...
			for (i = 0;
					i < (((long) samples_per_echo * (long) echoes_per_scan));
					i++) {
				fprintf(fptr, "%d\n", rddata_16[i]);
			}
			fclose (fptr);
...
void close_system() {
	...
	write_i2c_cnt(DISABLE,
			PAMP_IN_SEL_TEST_msk | PAMP_IN_SEL_RX_msk | PSU_15V_TX_P_EN_msk
					...
					| PSU_5V_TX_N_EN_msk, DISABLE_MESSAGE);
...
int main(int argc, char * argv[]) {
	...
	rddata_16 = (unsigned int*) malloc(
			samples_per_echo * echoes_per_scan * sizeof(unsigned int)); //added malloc to this routine only - other routines will need to be updated when required
	rddata = (unsigned int *) malloc(
			samples_per_echo * echoes_per_scan / 2 * sizeof(unsigned int));	// petrillo 2Feb2019
